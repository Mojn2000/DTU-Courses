...
public class HelloServantListBasedListBased extends UnicastRemoteObject implements HelloServiceListBased {
	...
	private HashMap<String, HashSet<String>> methodToUser = new HashMap<>();
	public static String logFile = "C:\\Users\\remil\\Documents\\Repo\\lab2-02239\\lab2\\tmp\\.log";
	public static String accessFile = "C:\\Users\\remil\\Documents\\Repo\\lab2-02239\\lab2\\tmp\\.access";
	private static final SecureRandom secureRandom = new SecureRandom(); //threadsafe
	private static final Base64.Encoder base64Encoder = Base64.getUrlEncoder(); //threadsafe

    public HelloServantListBasedListBased() throws java.rmi.RemoteException {
        super();
		readAccessFile();
    }
...
//			writeToLogFile(String.format("Server Offline;SessionId=%s;Method=topQueue();TimeStamp=%s", sessionId, LocalDateTime.now().toString()));
			return "Print Server is Offline.";}
		if (hasAccess(sessionId, "topQueue")) {
			if(printers.containsKey(printer)){
...
//			writeToLogFile(String.format("Server Offline;UserId=%s;Method=topQueue();TimeStamp=%s", usr, LocalDateTime.now().toString()));
			return "Print Server is Offline.";}
		if (hasAccess(usr, pswd, "topQueue")) {
			if(printers.containsKey(printer)){
				if(printers.get(printer).contains(file)){
					printers.get(printer).remove(file);
					printers.get(printer).add(0,file);
...
//			writeToLogFile(String.format("Server Offline;SessionId=%s;Method=restart();TimeStamp=%s", sessionId, LocalDateTime.now().toString()));
			return "Print Server is Offline.";}
		if (hasAccess(sessionId, "restart")) {
...
//			writeToLogFile(String.format("Server Offline;SessionId=%s;Method=setConfig();TimeStamp=%s", sessionId, LocalDateTime.now().toString()));
			return "Print Server is Offline.";}
		if (hasAccess(sessionId, "setConfig")) {
...
//			writeToLogFile(String.format("Server Offline;UserId=%s;Method=setConfig();TimeStamp=%s", usr, LocalDateTime.now().toString()));
		...
		if (hasAccess(usr, pswd, "setConfig")) {
...
//			writeToLogFile(String.format("Server Offline;SessionId=%s;Method=readConfig();TimeStamp=%s", sessionId, LocalDateTime.now().toString()));
			return "Print Server is Offline.";}
		if (hasAccess(sessionId, "readConfig")) {
...
//			writeToLogFile(String.format("Server Offline;UserId=%s;Method=readConfig();TimeStamp=%s", usr, LocalDateTime.now().toString()));
		...
		if (hasAccess(usr, pswd, "readConfig")) {
...
//			writeToLogFile(String.format("Server Offline;UserId=%s;Method=authenticate();TimeStamp=%s", usr, LocalDateTime.now().toString()));
	...
	/**
	 * This method is used to check the status of a printer with authentication using password.
	 *
	 * @param printer Printer name
	 * @param usr Username of the user
	 * @param pswd Password of the user
	 * @return Returns the status of the printer.
	 * @throws RemoteException
	 */
...
//			writeToLogFile(String.format("Server Offline;UserId=%s;Method=status();TimeStamp=%s", usr, LocalDateTime.now().toString()));
	...
	/**
	 * This method is used to check the status of a printer with authentication using Session ID.
	 *
	 * @param printer Printer name
	 * @param sessionId Session ID of the user
	 * @return Returns the status of the printer.
	...
	/**
	 * This method is used to check if the user has access to a method with authentication using Session ID.
	 *
	 * @param sessionId Session ID of the user
	 * @param methodName Name of the method
	 * @return Returns true if the user has access else returns false.
	 */
	private Boolean hasAccess(String sessionId, String methodName){
		if (!sessions.containsKey(sessionId)) return false;

		var usr = sessions.get(sessionId);
		if(methodToUser.get("all").contains(usr) || methodName.equals("authenticate")) return true;
	...
	private void readAccessFile() {
		System.out.println("Reading access file");
		try {
			File text = new File(accessFile);
			...
			while (sc.hasNextLine()) {
				...
				method = line.split(":")[0];
				set = new HashSet<String>(Arrays.asList(line.split(":")[1].split(", ")));

				this.methodToUser.put(method, set);
	...
	public String editUsers(String sessionId, String methodToEdit, List<String> newUsers) throws IOException {

		if(!hasAccess(sessionId, "all")) return "Access Denied.";

		return editUsersHelper(methodToEdit, newUsers);
	}

	private String editUsersHelper(String methodToEdit, List<String> newUsers) throws IOException {

		boolean overwrite = false;
			//find user and append roles
		...
		while((line = br.readLine()) != null) {
			...
			if(name.equals(methodToEdit)) {
				overwrite = true;
				if (newUsers.size() == 0) continue;

				oldtext = methodToEdit + ":" ;
				for(int i = 0; i < newUsers.size(); i++) {
					if(i == newUsers.size() - 1) {
						oldtext += newUsers.get(i);
					} else {
						oldtext += newUsers.get(i) + ", ";
					}
				}
		...
		if (!overwrite) {
			if(newUsers.size() == 0) return "User does not exist and no new roles were specified.";
			System.out.println("Method not found");

			FileWriter fileWriter = new FileWriter(accessFile, true);
			PrintWriter printWriter = new PrintWriter(fileWriter);
			StringBuilder s = new StringBuilder(methodToEdit + ":");
			for(int i = 0; i < newUsers.size(); i++) {
				if(i == newUsers.size() - 1) {
					s.append(newUsers.get(i));
				} else {
					s.append(newUsers.get(i)).append(", ");
				}
			}
	...
	public String editUsers(String usr, String pwd, String methodToEdit, List<String> newUsers) throws IOException {

		if(!hasAccess(usr, pwd, "all")) return "Access Denied.";
	...
	public String removeAllUsers(String sessionId, String methodToEdit) throws IOException {
		return editUsers(sessionId, methodToEdit, new ArrayList<String>());
	}
	public String removeAllUsers(String usr, String pwd, String methodToEdit) throws IOException {
		return editUsers(usr, pwd, methodToEdit, new ArrayList<>());
	}

	public String addUsers(String sessionId, String methodToEdit, List<String> usersToAdd) throws IOException {
		if(!hasAccess(sessionId, "all")) return "Access Denied.";

		if(methodToUser.containsKey(methodToEdit)) {
			methodToUser.get(methodToEdit).addAll(usersToAdd);
		}
		else {
			methodToUser.put(methodToEdit, new HashSet<>(usersToAdd));
		}
		return editUsers(sessionId, methodToEdit, new ArrayList<>(methodToUser.get(methodToEdit)));
	}
	public String addUsers(String usr, String pwd, String methodToEdit, List<String> usersToAdd) throws IOException {
		...
		return editUsers(usr, pwd, methodToEdit, new ArrayList<>(methodToUser.get(methodToEdit)));
	}
	...
	public String removeUsers(String sessionId, String methodToEdit, List<String> usersToRemove) throws IOException {
		...
		if(methodToUser.containsKey(methodToEdit)) {
			usersToRemove.forEach(methodToUser.get(methodToEdit)::remove);
		}
	...
	public String changeUser(String sessionId, String userOld, String userNew) throws IOException {
		...
		this.methodToUser.forEach((k, v) -> {
			if(v.contains(userOld)) {
				v.add(userNew);
				v.remove(userOld);
				try {
					editUsers(sessionId, k, new ArrayList<>(methodToUser.get(k)));
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
			}
	...
	public String changeUser(String usr, String pwd, String userOld, String userNew) throws IOException {
		...
		this.methodToUser.forEach((k, v) -> {
			if(v.contains(userOld)) {
				...
				v.remove(userOld);
				try {
					editUsers(usr, pwd, k, new ArrayList<>(methodToUser.get(k)));
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
			}
