...
Atol = 10^(-2);
Rtol = Atol;
options = struct('Jac' , @VanPolJac,'step_control',true, 'initialStepSize',true,'control_type', "PI",'Rtol',Rtol,'Atol',Atol);

tic
[X1,T1,function_calls1,hs1sol,rs1] = ODEsolver(@VanPol,[mu],h,t0,tend,x0, "ESDIRK23", options);
time1 = toc;

options.Atol = 10^(-4);
options.Rtol = options.Atol;
tic
[X2,T2,function_calls2,hs2sol,rs2] = ODEsolver(@VanPol,[mu],h,t0,tend,x0, "ESDIRK23", options);
time2 = toc;

options.Atol = 10^(-6);
options.Rtol = options.Atol;
tic
[X3,T3,function_calls3,hs3sol,rs3] = ODEsolver(@VanPol,[mu],h,t0,tend,x0, "ESDIRK23", options);
time3 = toc;
...
T2 = hs2sol.t;
T3 = hs3sol.t;
hs1 = [hs1sol.h; hs1sol.r];
hs2 = [hs2sol.h; hs2sol.r];
hs3 = [hs3sol.h; hs3sol.r];
subplot(1, 3, 1)
