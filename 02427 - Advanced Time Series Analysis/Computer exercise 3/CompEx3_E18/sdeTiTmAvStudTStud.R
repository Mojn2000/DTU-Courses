...
## Adds to thermal mass when student show up

sdeTiTmAvStudTStud <- function(data, yTi,Ph){
  data$yTi <- yTi
  data$Ph <- Ph
  ...
  model$addSystem(dTi ~  1/Ci*(1/Ria*(Ta-Ti) + 1/Rim*(Tm-Ti)  + ha*Ph + (aw1*bs1 + aw2*bs2 + aw3*bs3 + aw4*bs4 + aw5*bs5)*Gv)*dt + (stud*sigmaStud + exp(p11))*dw1)
  model$addSystem(dTm ~  1/(Cm+Cstud*stud)*(1/Rim*(Ti-Tm)+(1-ha)*Ph)*dt + exp(p22)*dw2)
  ...
  model$setParameter(Cstud = c(init = 10, lb = 1E-5, ub = 1E5))
