...
sdeTiTmAvVarHeat <- function(data, yTi,Ph){
  data$yTi <- yTi
  data$Ph <- Ph
  ...
  model$addSystem(dTi ~  1/Ci*(1/Ria*(Ta-Ti) + 1/Rim*(Tm-Ti) + rho*Ph + (aw1*bs1 + aw3*bs3)*Gv)*dt + exp(p11)*dw1)
  ...
  model$setParameter(rho = c(init = 1, lb = 0, ub = 10))
