...
## Include 9'oclock factor

sdeTiTmAvStudMeet <- function(data, yTi,Ph){
  data$yTi <- yTi
  data$Ph <- Ph
  ...
  model$addSystem(dTi ~  1/Ci*(1/Ria*(Ta-Ti) + 1/Rim*(Tm-Ti) + ha*Ph + (aw1*bs1 + aw2*bs2 + aw3*bs3 + aw4*bs4 + aw5*bs5)*Gv + meet*meetIn*(Ta-Ti))*dt + (stud*sigmaStud + exp(p11))*dw1)
  model$addSystem(dTm ~  1/Cm*(1/Rim*(Ti-Tm)+(1-ha)*Ph)*dt + exp(p22)*dw2)
  # Set the names of the inputs
  model$addInput(Ta,Gv,Ph,bs1,bs2,bs3,bs4,bs5,stud,meetIn)
  ...
  model$setParameter(meet = c(init = 1, lb = -5, ub = 5))
